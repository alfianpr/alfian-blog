(ns blog-proto.render
  (:require [clojure.string :as str]
            [me.raynes.fs :as fs]
            [selmer.parser :as tmpl]
            [taoensso.timbre :as log]
            [com.climate.claypoole :as cp]
            [blog-proto.adoc :as adoc]))
...
(defn- source->post
  ...
  (let [{:keys [title date html] :as post} (adoc/parse source)]
    (-> post
        (merge post
               {:slug (fs/name file)
                :page-html (tmpl/render-file "post.html" {:title title :date date :content html})}))))
...
(defn render!
  ...
  (let [posts (->> (fs/list-dir posts-dir)
    ...
    (doall
     (cp/pcalls 4 #(spit-index! posts) #(spit-sitemap! posts) #(spit-feed! posts) #(spit-404!)))
